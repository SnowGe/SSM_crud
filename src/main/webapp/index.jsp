...
%>
<!-- 
web 路径，不已/开始的相对路径，找资源，以当前资源为基准，经常容易出问题。
以/开始的相对路径，找资源，以服务器的路径为标准(http://localhost:3306)；需要加上项目名字
      ...
      <div class="modal-body">
		<form class="form-horizontal">
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-10">
		      <input type="text" class="form-control" name="emaill" id="email_update_input" placeholder="email@atguigu.com">
      ...
      <div class="modal-body">
		<form class="form-horizontal">
		  ...
		  <div class="form-group">
		    <label for="email_add_input" class="col-sm-2 control-label">email</label>
		    <div class="col-sm-10">
		      <input type="text" class="form-control" name="emaill" id="email_add_input" placeholder="email@atguigu.com">
		      <span class="help-block"></span>
		    </div>
		  </div>
		  <div class="form-group">
	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(emps,function(index,item){
				...
				var genderTd = $("<td></td>").append(
						item.gender == 'M' ? "男" : "女");
				var emailTd = $("<td></td>").append(item.emaill);
		...
		function getDepts(ele){
			...
			$.ajax({
				...
				type:"GET",
				success:function(result){
					//{"code":100,"message":"处理成功","extend":{"depts":[{"deptId":1,"deptName":"开发部"},{"deptId":2,"deptName":"测试部"}]}}
					//console.log(result);
					//显示部门信息在下拉列表中
					//$("#dept_add_select")
					...
					$.each(result.extend.depts, function(){
						//this可以表示当前被遍历的对象，function()表示回调函数
		...
		function validate_add_form(){
			...
			if (!regName.test(empName)){
				//alert("用户名可以是2-5未中文或者6-16位英文和数字的组合");
				show_validate_msg("#empName_input", "error", "用户名可以是2-5未中文或者6-16位英文和数字的组合");
				//$("#empName_input").parent().addClass("has-error");
				//$("#empName_input").next("span").text("用户名可以是2-5未中文或者6-16位英文和数字的组合");
				return false;
			}else{
			...
			$.ajax({
				...
				success:function(result){					
					...
					if(result.code ==100){
						...
						//2.来到最后一页，显示刚保存的数据。
						//发送ajax请求，显示最后一页。
						to_page(totalRecord);//to_page(9999);
					}else{
						//显示失败信息
						//alert(result);
						//console.log(result);
						//有哪个字段的错误信息就显示哪个
						if(undefined != result.extend.errorFields.emaill){
							//显示邮箱错误信息
							show_validate_msg("#email_add_input", "error", result.extend.errorFields.emaill);
						}
		...
		//我们是在按钮创建之前就绑定了click事件，所以绑定不上。
		//workround:1)、可以在创建按钮的时候绑定事件。2）绑定.live()----Jquery新版本没有live，使用on进行替代
		$(document).on("click", ".edit_btn", function(){
			//alert("edit");
			//1.弹出模态框之前，查出员工信息，显示员工信息，并且查询部门信息，显示部门信息
			//查出员工信息
			 //alert($(".edit_btn").attr("edit-id"));
		...
		function getEmp(id){
			$.ajax({
				...
				success:function(result){
					...
					$("#email_update_input").val(empData.emaill);
		...
		$(document).on("click",".delete_btn", function(){
			...
			var empId = $(this).attr("del-id").val();
			if(confirm("确认删除【"+ empName+ "】么？")){
				//确认。发送ajax请求删除
				$.ajax({
					url:"${APP_PATH}/emp/" +empId,
					type:"DELETE",
					sueecee:function(result){
						alert(result.message);
						//回到本页
						to_page(currentPage);
					}
		...
		$("#check_all").click(function(){
			...
			//我们这些dom原生的属性，使用prop修改和读取dom原生属性的值，attr获取自定义的值
